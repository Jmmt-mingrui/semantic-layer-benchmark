...
1. 与空白上下文和 DDL-only 对照组相比，语义上下文能否提升结果执行准确率？
...
| Cube | 语义层服务和查询 API | Agent 发现可用成员，再向运行中的 Cube 服务提交一个明确声明的原生查询变体 | 是 |
| Apache Ossie | 语义模型交换规范 | Validator 和参考适配器加载 Ossie 文档；Agent 消费模型，互操作能力单独测试 | 不假设存在独立运行时 |
| OKF | 可移植知识表示 | Agent 直接浏览原始 Markdown、Frontmatter、Index 和链接，再编写 SQL | 否 |
| Skill | Agent 工作流和渐进式上下文 | Agent 激活 `SKILL.md`，按需加载引用知识，然后编写 SQL | 否 |
| DDL-only | 对照条件 | 同一个 Agent 只获得物理 DDL 和问题 | 由 Agent 直接编写 |
| 空白上下文 | 对照条件 | 全新 Agent 不预加载 Schema 或语义，只能自行发现原生数据库 Catalog | 由 Agent 直接编写 |
...
flowchart TB
    ...
    C --> M["原生模型构建与校验"]
    Q --> R["Native-first 实验编排器"]
    M --> R
    D --> R

    subgraph target["每个 Trial 只运行一个隔离条件"]
        E["MetricFlow / Cube 原生 API"]
        K["Skill / OKF 原生消费"]
        O["Ossie 原生文档"]
        B["空白上下文 / DDL-only"]
    end
...
Gold SQL、预期结果和 question-to-metric map 只允许评估器访问，被测对象永远不能看到。每次 Trial 只暴露一个实例化问题，以及为该目标声明的产物和操作。可观测 Wrapper 可以记录原生调用，但不能将其改写成统一语义接口。
...
| 阶段与赛道 | 回答的问题 | 参与对象 | 输出 |
| --- | --- | --- | --- |
| Phase 1A：原生端到端 | 按照每种对象的预期方式使用，实际效果如何？ | 五种对象加空白上下文和 DDL-only 对照 | 原生请求、可观测的生成 SQL 和最终结果 |
| Phase 1B：原生 Serving | 确定性语义编译和 Serving 的开销如何？ | 仅 MetricFlow 和 Cube | 编译或服务结果、冷/热耗时及引擎诊断 |
| Phase 1C：受控上下文消融 | 去掉原生 Runtime 后，抽取出的语义信息本身是否有帮助？ | 仅明确声明的次要变体 | 使用归一化 Evidence 生成 DuckDB SQL，不能被描述为原生行为 |
...
这是主结果。每个目标都保留自己的原生产物、发现行为和执行接口：

- MetricFlow：Agent 选择 Metric、Dimension、Filter 和时间范围，再由 MetricFlow 编译 SQL。
- Cube：REST 变体中，Agent 查询 `/v1/meta`，然后调用 `/v1/load`；Semantic SQL 作为另一个原生变体单独测量。
- Skill：Agent 激活 Skill，只按需加载相关指令和 Concept。
- OKF：Agent 直接浏览原始 Markdown Bundle、YAML Frontmatter、Index 和链接，然后编写 SQL。
- Ossie：Consumer 向 Agent 暴露已校验的原始 Ossie YAML，再由 Agent 编写 SQL；通过其他引擎执行属于独立互操作变体。
- DDL-only：Agent 可以看到 DuckDB 物理 DDL，但没有增强业务语义，然后编写 SQL。
- 空白上下文：Agent 初始看不到表、列、指标或文档，只能自行发现 DuckDB Catalog 并编写只读 SQL。

每个 Agent Trial 都使用全新对话，并固定公共 Prompt 外壳和生成策略。原生操作 Allowlist 随目标变化；未声明的直接 SQL Fallback 记为协议违规。

### 受控上下文消融

归一化 `SemanticEvidence` Chunk 只能用于单独命名的次要实验。它可以研究剥离原生行为后的表示内容，但得分不能混入或替代原生结果。
...
| Cube | Cube YAML 或 JavaScript Data Model | `/v1/meta` 加每个变体明确声明的一个原生查询 API | Cube Runtime | BI、嵌入式分析和 Agent 的语义 API |
| Ossie | 包含 Dataset、Field、Relationship 和 Metric 的完整 YAML 文档 | 原生 Schema 校验和原始文档检查 | 默认由参考 Agent；互操作变体除外 | 在兼容工具之间迁移语义模型 |
| OKF | 带 YAML Frontmatter 和 Index 的 Markdown Concept | 直接浏览文件、Frontmatter、Index 和链接 | 参考 Agent | 可移植业务上下文、出处和整理后的知识 |
| Skill | `SKILL.md` 及可选 Reference 和 Script | 原生激活及渐进式文件读取 | 参考 Agent | 可重复执行的 Agent 指令和特定任务知识 |
| 空白上下文 | 不预加载 Schema 或语义 | 原生数据库 Catalog 发现和只读 SQL | 参考 Agent | 零预加载上下文基线 |
...
3. 加载一个实例化问题，但将参考 SQL、参考结果和 Canonical 映射留在目标 Sandbox 之外。
4. 使用目标准确的原生操作 Allowlist，启动全新 Provider Conversation 和 Root Trace。
5. 保存完整脱敏消息序列、原生请求、工具结果和可观测的生成 SQL。
6. 只接受一次最终结果提交；拒绝写操作、未声明的 Fallback，以及问题契约之外的多语句 SQL。
7. 关闭目标 Sandbox 后，才加载 Evaluator-only 产物并比较归一化结果。
8. 在同一个 `run_id` 和 `trial_id` 下保存 Trace、产物、Hash、用量、错误分类和评分。
...
- **原生主赛道不归一化：** 主赛道保留每个目标的原生产物和预期接口。公共 Chunking 与 Retrieval 只能出现在单独标记的消融赛道。
...
| `runner/examples/` | 非结果性质的协议示例和说明性 Transcript |
