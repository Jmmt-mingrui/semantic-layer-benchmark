...
| --- | --- | --- | --- |
| MetricFlow | Executable metric compiler | Query metrics and dimensions; compile or execute generated SQL | Primary native runtime comparison; optional ablation is separate |
| Cube | Executable semantic service | Query model metadata and one declared native query interface | Primary native runtime comparison; optional ablation is separate |
...
| Cube | `cube/` (not created yet) | Planned; no native model should be inferred from a placeholder |
...
| DDL-only | Physical schema under [`../data/tpcds/schema/duckdb/`](../data/tpcds/schema/duckdb/) | Control uses the original DDL rather than another semantic model copy |
...
- Execute through the pinned self-hosted MetricFlow query command and capture compiled SQL through its native compile/explain option when available.
- Evaluate the result returned by MetricFlow; a direct-SQL runner fallback is forbidden in the native condition.
...
- In the native end-to-end lane, expose the original validated YAML through a transparent benchmark consumer and let the reference agent write SQL.
- Convert it to common evidence chunks only in the separately labeled controlled-context ablation.
...
- Start at `index.md`, read the original Markdown and YAML frontmatter, and follow relevant links directly.
- Do not build a shared embedding index or convert the bundle into common chunks in the primary native condition.
...
OKF v0.2 intentionally prescribes a portable Markdown-plus-frontmatter corpus rather than storage, serving, or query infrastructure. The benchmark consumer supplies allowlisted file operations without changing the documents; OKF itself is not credited with generating SQL.
...
Official OpenAI documentation describes Skills as instruction/resource/script packages and uses progressive disclosure: initial metadata is visible first, then the complete `SKILL.md` is loaded when selected. The primary native Skill lane preserves this behavior. Any experiment that removes activation behavior is a separately labeled ablation.
...
## Optional controlled-context ablation

Only in Phase 1C, every format may be transformed into immutable evidence records containing:
...
The same chunker, search index, top-k, tie-breaking rule, and token cap are used for every ablation target. Retrieval output is part of the trace. Reference SQL, expected results, question-to-metric mappings, and evaluator annotations are never indexed. These secondary scores are never reported as native behavior.
